...
const init = function(debug, debugHost) {    
    ...
    this.generate_key_from_password = function(password) {
        ...
        return Buffer.from(hash.digest().slice(0, 32)) // key for aes-256 must be 32 bytes
    }
    ...
    this.upload_file_for_record = function(file, fileContent, recordID, password, projectID, handleProgress) {
        return new Promise((resolve, reject) => {
            ...
            let keyContent = JSON.stringify({key: cipherInfo.key, iv: this.iv_to_string(cipherInfo.iv)})
            let encryptedKey = this.encrypt_string_with_password(keyContent, password)

            console.log(encryptedKey.ciphertext)
            console.log(encryptedKey.iv)
            console.log(this.iv_to_string(encryptedKey.iv))
            
            let decryptKey = this.generate_key_from_password(password) 
            let decrypted = this.decrypt_string(encryptedKey.ciphertext, decryptKey, encryptedKey.iv)
    ...
    this.download_file_for_recordID = function(recordID, password, asPlaintext) {
        return new Promise((resolve, reject) => {
            ...
            .then(fileInfo => {
                ...
                console.log(this.decrypt_string(encryptedKey, decryptKey, iv))                
